...
+-- Step 3.5-5. 물리적 정확성 규칙 정의 (최소 5항목)
    +-- 형상 제약: "X는 반드시 Y 형태, Z 금지"
    +-- 통합/은폐 제약: "X는 Y 내부에 은폐/통합"
    +-- 장착 제약: "X는 Y의 양면에 장착"
    +-- 식별 특징 제약: "X는 반드시 Y 특징 필수"
    +-- 스케일 제약: "전체 비율이 X급에 적합"
```

**완전성 검증 규칙:**
- Domain Keywords의 **모든 물리적 대상** → Physical Objects에 독립 항목
- Physical Objects의 **모든 대상 쌍** → Physical Connections에 결합 기술
- Physical Objects의 **모든 대상** → Scale Relationships에서 크기 위계 언급
- Critical Accuracy Rules → **형상, 통합/은폐, 장착, 식별, 스케일** 5개 범주 포함
