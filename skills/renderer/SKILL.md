...
## 참조 이미지 합성 (Reference Image Compositing)

기존에 생성된 이미지들을 참조하여 물리적으로 자연스럽게 결합된 통합 이미지를 생성하는 기능이다.
Gemini API의 멀티모달 입력(이미지+텍스트)을 활용하여, 참조 이미지들의 시각 요소를 분석하고 하나의 이미지로 재합성한다.

### 프롬프트 마크다운 작성법

프롬프트 `.md` 파일에 `[REFERENCE IMAGES]` 섹션을 추가한다:

```markdown
==================================================
[REFERENCE IMAGES - DO NOT RENDER IN IMAGE]
==================================================

Combination Instruction: 아래 참조 이미지들의 구성품들이 물리적으로 연결된 전체 시스템 이미지를 생성하라.

- ../figures/02_AGB_concept.png
- ../figures/05_IGB_concept.png
- ../figures/08_RDS_concept.png
```
...
| 항목 | 설명 |
|------|------|
| 섹션 헤더 | `[REFERENCE IMAGES` 포함 (대소문자 무관) |
| 이미지 경로 | `- 상대경로.png` (프롬프트 파일 기준 상대 경로) |
| 지원 확장자 | `.png`, `.jpg`, `.jpeg`, `.webp` |
| Combination Instruction | 선택 사항. 미지정 시 기본 결합 지시문 사용 |
| 섹션 구분 | `====` 구분선으로 시작/종료 |

### 동작 방식

1. 프롬프트 파일에서 `[REFERENCE IMAGES]` 섹션 감지
2. 참조 이미지 경로 추출 → PIL Image로 로드
3. 결합 지시문 + 원본 프롬프트를 하나의 텍스트로 구성
4. `[참조 이미지들, 결합 텍스트]`를 Gemini API에 멀티모달 입력으로 전달
5. 참조 이미지의 시각 요소가 통합된 새 이미지 생성

### 실행 로그 예시

```
[1/3] 14_통합시스템_합성
  [참조] 3개 참조 이미지 감지: 02_AGB_concept.png, 05_IGB_concept.png, 08_RDS_concept.png
  [참조] 3개 로드 완료 → 멀티모달 합성 모드
  [OK] Saved: figures/14_통합시스템_합성.png
```

### 하위 호환성

- `[REFERENCE IMAGES]` 섹션이 없는 기존 프롬프트는 변경 없이 동일하게 동작
- 참조 이미지 파일이 존재하지 않으면 경고 출력 후 텍스트 전용 모드로 폴백
